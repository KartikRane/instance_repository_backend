...
class Connector:
    ...
    def __init__(
        ...
        self.logger = logging.getLogger(problem_uid)
    ...
    def _request(
        ...
        if json is not None:
            self.logger.info("    json_payload=%s", json)
        if files:
        ...
        self.logger.info("<-- [%d] %s", response.status_code, response.url)
        text = response.text
        if len(text) > 500:
            self.logger.info("    response_text=%s... [truncated]", text[:500])
        else:
            self.logger.info("    response_text=%s", text)
